...
// 光学迷彩処理の為にステージモデルを描画するための画面
int TempDrawScreen ;

// 光学迷彩効果でスキニングメッシュを描画するための頂点シェーダーとピクセルシェーダー
int Skin4_OpticalCamouflage_VertexShader ;
int SKin4_OpticalCamouflage_PixelShader ;
...
void CharaCommon_Initialize( void )
{
	...
	// 光学迷彩処理の為にステージモデルを描画するための画面を作成
	TempDrawScreen = MakeScreen( 512, 512, FALSE ) ;

	// 光学迷彩効果でスキニングメッシュを描画するための頂点シェーダーとピクセルシェーダーの読み込み
	Skin4_OpticalCamouflage_VertexShader = LoadVertexShader( "SkinMesh4_OpticalCamouflageVS.vso" ) ;
	SKin4_OpticalCamouflage_PixelShader = LoadPixelShader( "SkinMesh4_OpticalCamouflagePS.pso" ) ;
}
...
void Render_Process( void )
{
	...
	// 光学迷彩処理用の画面にステージモデルを描画

	// 描画先を光学迷彩処理用の一時画面へ変更
	SetDrawScreen( TempDrawScreen ) ;

	// 一時画面をクリア
	ClearDrawScreen() ;

	// カメラの設定を行う( SetDrawScreen でカメラ設定がリセットされてしまうので )
	SetCameraPositionAndTarget_UpVecY( cam.Eye, cam.Target ) ;

	// カメラに移す映像のアスペクト比を変更して 640x480 と範囲描画するようにする
	SetCameraDotAspect( 480.0f / 640.0f ) ;
	...
	// 光学迷彩シェーダーを使用してキャラクターを描画

	// オリジナルシェーダーを使用して描画する設定に変更
	MV1SetUseOrigShader( TRUE ) ;

	// 使用する頂点シェーダーをセット
	SetUseVertexShader( Skin4_OpticalCamouflage_VertexShader ) ;

	// 使用するピクセルシェーダーをセット
	SetUsePixelShader( SKin4_OpticalCamouflage_PixelShader ) ;

	// 光学迷彩処理用の画面画像を使用テクスチャ０にセット
	...
	// 光学明細書利用の画面画像を使用するテクスチャから外す
	SetUseTextureToShader( 0, -1 ) ;

	// オリジナルシェーダーを使用する設定を解除
